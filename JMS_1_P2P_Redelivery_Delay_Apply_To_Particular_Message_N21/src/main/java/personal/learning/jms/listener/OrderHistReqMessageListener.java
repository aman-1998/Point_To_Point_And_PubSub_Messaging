...
public class OrderHistReqMessageListener implements MessageListener {
	...
	public OrderHistReqMessageListener(Session session, String consumerName) {
	...
	public void onMessage(Message message) {
		...
		try {
			...
			if(StringUtils.equalsIgnoreCase(consumerName, "OrderHistReqConsumer1")) {
				if(exceptionFlag == 1) {
			...
			if(StringUtils.equalsIgnoreCase(consumerName, "OrderHistReqConsumer2") || exceptionFlag > 1) {
				session.commit();
			}
